...
|:------ |:-----|:-----------------|
| **cpu**| `string` | The number of CPU units used by the task. It can be expressed as an integer using CPU units, for example 1024, or as a string using vCPUs, for example 1 vCPU or 1 vcpu, in a task definition.
| **memory**| `string` | The amount of memory (in MiB) used by the task. It can be expressed as an integer using MiB, for example 1024, or as a string using GB, for example 1GB or 1 GB, in a task definition.
...
| **requiresAttributes**| `array` | The container instance attributes required by your task. This field is not valid if using the Fargate launch type for your task.
...
| **requiresCompatibilities**| `array` | The launch type the task is using.
