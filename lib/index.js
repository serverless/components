...
const { readFile, fileExists, readStateFile, writeStateFile, log } = utils
...
const generateContext = (componentId, options, graph) => {
  const context = {
    componentId,
    log,
    readStateFile,
    ...
    load: async (type, alias) => {
      ...
      let childComponentState = graph.state[childComponentId]|| {}
      const childComponentContext = generateContext(childComponentId, options, graph)

      const fnNames = keys(component)

      const modifiedComponent = reduce((accum, fnName) => {
        const childComponentFn = component[fnName]
        accum[fnName] = async (inputs) => {
          const childComponentOutputs = await childComponentFn(inputs, options,
            childComponentState, childComponentContext)
          childComponentState = mergeDeepRight(inputs, childComponentOutputs || {})
          graph.state[childComponentId] = childComponentState

          return childComponentState
        }
        return accum
      }, {}, fnNames)
...
const executeComponent = async (componentId, graph, command, options) => {
  ...
  if (typeof component.fns[command] === 'function') {
    const context = generateContext(componentId, options, graph)
    component.outputs = (await component.fns[command](inputs, options, state, context)) || {}
  }
...
const execute = async (graph, command, options) => {
  ...
  return execute(graph, command, options)
}
...
module.exports = async (command, options) => {
  ...
  try {
    ...
    await writeStateFile(graph.state)
  }
}
